...
/*   Updated: 2025/07/14 09:13:25 by eamchart         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	parse_paths(t_data *data, int i)
{
	...
	if ((ft_strncmp(exten, ".xpm", 4) != 0) || (ft_strncmp(configs[1], ".xpm", 4) == 0))
...
void	parse_color(t_data *data, int i)
{
	...
	while (configs[numb] != NULL)
		numb++;
	if (numb != 2 || ft_strlen(configs[0]) != 1 || configs[1][0] == ',' || configs[1][ft_strlen(configs[1]) - 1] == ',')
		free_all(data, configs, NULL, "❌ Error\nColor identifier has no or more arguments");
